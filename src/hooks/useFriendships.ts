...
export function getFriendshipState(friendship: Friendship | null | undefined, currentUserId?: string): FriendshipState {
...
export function useFriends(userId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) throw error;
      const ids = (data || []).map((row) => row.requester_id === userId ? row.addressee_id : row.requester_id);
...
function useFriendMutation(action: (input: { currentUserId: string; otherUserId: string; friendship?: Friendship | null }) => Promise<void>) {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ otherUserId, friendship }: { otherUserId: string; friendship?: Friendship | null }) => {
...
export function useSendFriendRequest() {
  return useFriendMutation(async ({ currentUserId, otherUserId }) => {
    const { error } = await supabase.from("friend_requests").insert({ requester_id: currentUserId, addressee_id: otherUserId });
    if (error) throw error;
  });
}
...
export function useAcceptFriendRequest() {
  return useFriendMutation(async ({ friendship }) => {
    ...
    const { error } = await supabase.from("friend_requests").update({ status: "accepted", updated_at: new Date().toISOString() }).eq("id", friendship.id);
    if (error) throw error;
  });
}

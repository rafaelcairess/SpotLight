...
export function useProfileComments(profileUserId?: string, enabled = true) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.from("profile_comments").select("*").eq("profile_user_id", profileUserId).order("created_at", { ascending: false }).limit(100);
      ...
      return (data || []).map((comment) => ({
        ...comment,
        author: authors.get(comment.author_id) || { username: "jogador", display_name: "Jogador", avatar_url: null },
      })) as ProfileComment[];
    },
...
export function useAddProfileComment() {
  ...
  return useMutation({
    mutationFn: async ({ profileUserId, content }: { profileUserId: string; content: string }) => {
      ...
      const { error } = await supabase.from("profile_comments").insert({ profile_user_id: profileUserId, author_id: user.id, content: cleanContent });
      if (error) throw error;
    },
    onSuccess: (_, variables) => queryClient.invalidateQueries({ queryKey: ["profile-comments", variables.profileUserId] }),
  });
}

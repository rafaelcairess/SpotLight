...
export function useFavoriteGame(profileUserId?: string, appId?: number | null) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: ownership, error: ownershipError } = await supabase.from("user_games").select("app_id, hours_played").eq("user_id", profileUserId).eq("app_id", appId).maybeSingle();
      if (ownershipError) throw ownershipError;
      if (!ownership) return null;
      const { data, error } = await supabase.from("games").select("*").eq("app_id", appId).maybeSingle();
      if (error) throw error;
      return data ? { ...(mapGameRow(data) as GameData), hoursPlayed: ownership.hours_played } : null;
    },

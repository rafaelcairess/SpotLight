...
export function SignupForm({ onSwitchToLogin }: SignupFormProps) {
  ...
  const { signUp, signInWithGoogle, signInWithSteam, signInWithXbox } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      toast.error(t("auth.form.signupError"), { description: error.message });
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {/* Cadastro/login rápido via plataforma */}
      <div className="space-y-2">
        {/* Steam */}
        ...
        {/* Google — em construção */}
        <Button type="button" variant="outline" className="w-full gap-2 opacity-50 cursor-not-allowed" disabled>

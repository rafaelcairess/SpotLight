  ...
  const moveScreenshot = useCallback(
    (direction: number) => {
      ...
      setSelectedScreenshot(
        screenshots[(selectedIndex + direction + screenshots.length) % screenshots.length],
      );
    },
  ...
  if (loading && !game.trailerUrl && !screenshots.length) {
    return (
      <section className="space-y-3" aria-busy="true">
        ...
        <div className="aspect-video animate-pulse rounded-lg bg-secondary/60" />
        <div className="grid grid-cols-3 gap-2">
          {[0, 1, 2].map((item) => (
            <div key={item} className="aspect-video animate-pulse rounded-md bg-secondary/40" />
          ))}
        </div>
      </section>
    );
      ...
      {game.trailerUrl && (
        <div className="relative aspect-video overflow-hidden rounded-lg bg-black">
          {playing ? (
            ...
            >
              <img
                src={game.trailerThumbnail || game.image}
                alt=""
                fetchPriority="high"
          ...
          {screenshots.map((url, index) => (
            ...
            >
              <img
                ...
                loading="lazy"
                decoding="async"
                className="aspect-video h-full w-full object-cover transition-transform hover:scale-[1.03]"
              />
            </button>
          ))}
        </div>
      ...
      {selectedScreenshot &&
        createPortal(
          <div
            role="dialog"
            aria-modal="true"
            aria-label="Captura de tela ampliada"
            className="pointer-events-auto fixed inset-0 z-[100] grid place-items-center bg-black/40 p-4 backdrop-blur-[1px]"
            onClick={() => setSelectedScreenshot(null)}
          >
            <button
              type="button"
              onClick={() => setSelectedScreenshot(null)}
              className="absolute right-5 top-5 z-20 grid h-12 w-12 place-items-center rounded-full bg-black/75 text-white shadow-lg hover:bg-black"

...
export function RecentActivity({ userId }: { userId?: string }) {
  ...
  const recentHours = visibleActivity.reduce(
    (total, item) => total + Number(item.playtime_2weeks || 0),
    0,
  );
      ...
      <header className="flex flex-wrap items-center justify-between gap-2 border-b border-white/5 bg-primary/10 px-4 py-3">
        ...
        {recentHours > 0 && (
          <span className="text-xs text-muted-foreground">
            {formatHours(recentHours)} horas nas últimas 2 semanas
          </span>
        )}
      </header>
          ...
          return (
            ...
            >
              <img
                src={
                  game?.image ||
                  `https://cdn.cloudflare.steamstatic.com/steam/apps/${item.app_id}/header.jpg`
                }
                alt=""
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  {formatHours(item.hours_played)} horas registradas
                </p>
                {playedAt && (
                  <p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
                    <Clock3 className="h-3.5 w-3.5" />
                    Jogado em {playedAt.toLocaleDateString("pt-BR")}
                  </p>
                )}
              </div>
            </article>

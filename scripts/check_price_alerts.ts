...
const extractPriceSnapshot = (details: SteamDetails | null): PriceSnapshot | null => {
...
const main = async () => {
  ...
  const uniqueGameIds = Array.from(new Set(typedAlerts.map((alert) => alert.game_id)));
  const priceSnapshots = new Map<number, PriceSnapshot>();
  ...
  for (const gameId of uniqueGameIds) {
    try {
      ...
      console.error(`Erro ao consultar ${gameId}:`, err instanceof Error ? err.message : err);
    }
